...
minetest.register_on_joinplayer(function(player)
	...
	if fishing_setting.contest["contest"] == true then
		minetest.chat_send_player(player_name, S("A Beer drinking contest is in progress. (remaining time %s)"):format(fishing_setting.func.timetostr(fishing_setting.contest["duration"])))
	end
end)
...
function fishing_setting.func.end_contest()
	...
	minetest.chat_send_all(S("End of beer drinking contest."))
	...
	if formname == "fishing:contest" then
		...
		if fields["save"] then
			...
			if progress == false and fishing_setting.tmp_setting["contest"] == true then
				...
				minetest.chat_send_all(S("Attention, Beer drinking contest start (duration %s)!!!"):format(time))
...
function fishing_setting.func.get_stat()
	...
	local formspec = {"size[6,8]label[2,0;"..S("Beer drinking contest rankings").."]"}
	local Y = 1.1
...
function fishing_setting.func.tick()
	if fishing_setting.contest["contest"] ~= nil and fishing_setting.contest["contest"] == true then
		...
		if fishing_setting.contest["duration"] < 30 and fishing_setting.contest["warning_said"] ~= true then
			minetest.chat_send_all(fishing_setting.func.S("WARNING, Beer drinking contest will finish in 30 seconds."))
