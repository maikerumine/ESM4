...
radiant_damage.register_radiant_damage("mese", {
	...
	inverse_square_falloff = true,
	emitted_by = {},
--	["es:toxic_water_flowing"] = radiant_damage.config.mese_damage,
--	["es:toxic_water_source"] = radiant_damage.config.mese_damage * 3,--},	--was9
--	["es:depleted_uraniums"] = radiant_damage.config.mese_damage * 3,
--	["es:stone_with_purpelliums"] = radiant_damage.config.mese_damage * 2,
--	["es:toxic_container"] = radiant_damage.config.mese_damage * 3,
--	["es:stone_with_infiniums"] = radiant_damage.config.mese_damage * 2},
	attenuated_by = {
	["group:stone"] = 0.5, 
...
if radiant_damage.config.enable_radioactive_damage then
	local on_radioactive_damage
	if minetest.get_modpath("3d_armor") and armor ~= nil then
		-- 3d_armor uses a strange fire protection system different from all the rest, wherein
		-- its armor protects wholly against some heat sources and not at all against others
		-- based on how "hot" they are and how strong against fire the armor is.
		...
		-- y = 0.0481417 + 0.9518583*e^(-0.8388176*x)
		-- Which looks about right on a graph, and "looks about right on a graph" is good enough for me.
		on_radioactive_damage = function(player, damage, pos)
			local radioactive_protection = armor.def[player:get_player_name()].radiation
			local radioactive_multiplier = 1
			if radioactive_protection then
				radioactive_multiplier = math.min(1, 0.0481417 + 0.9518583 * math.exp(-0.8388176*radioactive_protection))
			end
			-- If the player also has conventional radiation armor, use whatever's better.
			local radiation_armor = player:get_armor_groups().radiation
			if radiation_armor then
				radioactive_multiplier = math.min(radioactive_multiplier, radiation_armor/100)
			end
			damage = math.floor(damage * radioactive_multiplier)
			if damage > 0 then
				minetest.log("action", player:get_player_name() .. " takes " .. tostring(damage) .. " damage from radioactive radiant damage at " .. minetest.pos_to_string(pos))
				player:set_hp(player:get_hp() - damage)
				minetest.sound_play("radiant_damage_geiger", {gain = math.min(1, damage/10), pos=pos})
			end
		end
	else
		on_radioactive_damage = function(player, damage, pos)
			local fire_armor = player:get_armor_groups().radiation
			if radiation_armor then
				damage = damage * radiation_armor / 100
			end
			damage = math.floor(damage)
			if damage > 0 then
				minetest.log("action", player:get_player_name() .. " takes " .. tostring(damage) .. " damage from radioactive radiant damage at " .. minetest.pos_to_string(pos))
